...
def get_region_end(region_string):
    if 'join' in region_string:
        return -5
    if 'complement' in region_string:
        return int(region_string.replace("complement(","").replace(")","").split('..')[1])
...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return (torch.tensor(self.X.iloc[idx]['Species ID']).float(),
                torch.tensor(self.X.iloc[idx]['Stress_name']).float(),
                torch.tensor(self.X.iloc[idx]['upstream200']).float().view(1,-1,4)), torch.tensor(self.y.iloc[idx]).float()