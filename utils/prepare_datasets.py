...
def stratified_split(df: pd.DataFrame, test_size: float = 0.2, val_size: float = 0.25, random_state: int = 42) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    ...
    """
    df['group'] = df['species_name'].astype(str) + "_" + df['stress_condition_name'].astype(str)
...
def prepare_tensors(df: pd.DataFrame) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    """
    sequence_tensor = torch.tensor(df['upstream200'].to_list(), dtype=torch.float32).permute(0, 2, 1)
...
def create_datasets_and_loaders(df_train: pd.DataFrame, df_val: pd.DataFrame, df_test: pd.DataFrame, batch_size: int) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """
...
def prepare_data_loaders(df: pd.DataFrame, batch_size: int) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """
    ...
    train_loader, val_loader, test_loader = create_datasets_and_loaders(df_train, df_val, df_test, batch_size)
    return train_loader, val_loader, test_loader
